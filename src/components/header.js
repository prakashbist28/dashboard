...
export default function Headertile({search, setSearch}) {
  return (
    <div className="flex flex-row items-center bg-white md:gap-4 p-1 pb-2 md:p-4 justify-between border-b-2 shadow-md shadow-slate-500/40 hover:shadow-xl duration-200">
      ...
      <div className="flex gap-4 md:gap-8 md:w-1/2 items-center">
  
        <input className="focus:border-blue-500 flex group w-[40%] md:w-1/2 items-center border-2 p-1 border-slate-400 rounded-lg lg:w-200px text-[12px] md:text-[14px] outline-none hover:border-blue-600 duration-300" value={search} onChange={(e)=> setSearch(e.target.value)} placeholder="🔍︎ search..."/>
