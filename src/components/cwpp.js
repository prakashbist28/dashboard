...
const CwppDashboard = ({ search }) => {
  const { dashboards, visibleCharts, addWidget, deleteWidget } =
    useContext(DashboardContext);

  const cwppCategory = dashboards.find((cat) => cat.id === "cwpp");
  ...
  const handleAddWidget = () => {
    ...
    addWidget("cwpp", widget);
    setNewWidget({ name: "", tags: "", tagValues: "", colors: "" });
    setShowForm(false);
  };

  const handleDeleteWidget = (id) => {
    deleteWidget("cwpp", id);
  };

  const filteredWidgets = cwppCategory.charts.filter((w) => {
      ...
      {showForm && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm z-50">
          <div className="bg-white p-6 shadow-lg rounded-xl max-w-md">
            <h3 className="text-xl font-bold mb-4">Add New Bar Chart</h3>
            <input
              type="text"
            ...
            <input
              ...
              onChange={(e) =>
                setNewWidget({ ...newWidget, tags: e.target.value })
              }
              className="w-full border p-2 mb-3 rounded"
            />
            <input
              type="text"
              placeholder="Tag Values (e.g. VMs:40, Containers:60)"
              value={newWidget.tagValues}
              onChange={(e) =>
                setNewWidget({ ...newWidget, tagValues: e.target.value })
              }
              className="w-full border p-2 mb-3 rounded"
            />
            <input
              ...
              onChange={(e) =>
                setNewWidget({ ...newWidget, colors: e.target.value })
              }
              className="w-full border p-2 mb-3 rounded"
            />
